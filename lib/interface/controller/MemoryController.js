...
class MemoryController {
  ...
  /**
   * Check freshness (POST /v1/memory/check-freshness)
   */
  async checkFreshness(req, res) {
    try {
      const checkFreshness = this.locator.get('CheckFreshness');
      const { query, group_id, max_age_hours } = req.body;
      ...
      res.json({
        success: true,
        data: results,
        count: results.split('\n').length - 2 // Exclude header lines
      });
    } catch (error) {
      logger.error({ err: error }, 'CheckFreshness error');
      res.status(400).json({
        success: false,
        error: error.message
      });
