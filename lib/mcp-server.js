...
If unsure → SAVE.`,
      ...
      {
        name: 'search_nodes',
        description: `Search knowledge graph. MUST USE FIRST before answering ANY question.

SEARCH PATTERNS (efficient queries):
1. "index:[project]" → Get project overview FIRST
2. "file:[project]:*" → Find all file summaries
3. "func:[project]:ClassName.*" → Find class methods
4. "api:[project]:*" → Find all API endpoints
5. "[project] [keyword]" → General search

WORKFLOW:
1. User asks about project → search "index:[project]" FIRST
2. Need specifics? → search "[type]:[project]:[scope]"
3. Empty result? → Explore codebase → add_memory immediately
4. Found? → Use results, DON'T re-read files
