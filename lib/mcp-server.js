...
- "What calls this function?"`,
      ...
      },
      {
        ...
        description: `Check if stored memories are fresh or stale. Use when:
- Code may have changed since memory was stored
- Memory is old (>24h) and you need current info
- You're unsure if stored knowledge is accurate

Returns entities with age and source file. Decide:
- FRESH (<24h): Trust the memory
- STALE (>24h): Consider re-reading file and updating memory`,
        inputSchema: {
          type: 'object',
          properties: {
            query: {
              type: 'string',
              description: 'Search query to find entities to check',
            },
            group_id: {
              type: 'string',
              description: 'Project name (kebab-case)',
            ...
            max_age_hours: {
              type: 'number',
              description: 'Mark as stale if older than this (default: 24)',
            },
          },
          required: ['query'],
        },
  ...
  try {
    switch (name) {
      ...
      case 'check_freshness': {
        const checkFreshness = Locator.get('CheckFreshness');
        const results = await checkFreshness.execute({
          query: args.query,
          group_ids: args.group_id ? [args.group_id] : [],
          max_age_hours: args.max_age_hours || 24,
        });
        return {
          content: [
            {
