#!/bin/bash
# Debounced async observe-read hook for allan-memory.
...
# latest read of a given file is processed — rapid successive reads
...
KEY="read-$(printf '%s' "$FILE_PATH" | shasum -a 256 | cut -d' ' -f1)"
